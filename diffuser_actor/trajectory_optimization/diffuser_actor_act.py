...
#整个3dda的框架
class DiffuserActorACT(nn.Module):
    ...
    def policy_forward_pass(self, trajectory, timestep, fixed_inputs):
        ...
        (
            context_feats,
            context,
            instr_feats,#torch.Size([10, 16, 192]) (B, max_instruction_length, F)
            adaln_gripper_feats,
            fps_feats,
            fps_pos
        ) = fixed_inputs
    ...
    def conditional_sample(self, condition_data, condition_mask, fixed_inputs):
        ...
        for t in timesteps:
            out, instr_feats = self.policy_forward_pass(
                trajectory,
    ...
    def forward(
        ...
        #这里gt_trajectoty是一整条轨迹，所以前面这个值就是轨迹的长度，后面的值是action的几个维度
        ...
        pred, instr_feats = self.policy_forward_pass(
            noisy_trajectory, timesteps, fixed_inputs
        )
        # # 设置设备
        # process_device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        # # # 定义线性层，将512维映射到192维
        # linear = nn.Linear(512, 192).to(process_device)
        # action_embedding_mapped = linear(action_embedding)
        # import pdb; pdb.set_trace()
        # Compute loss
        total_loss = 0
        for layer_pred in pred:
            trans = layer_pred[..., :3]
            rot = layer_pred[..., 3:9]
            trans_llm = act_pred[..., :3]
            rot_llm = act_pred[..., 3:9]
            loss = (
                30 * (F.l1_loss(trans, noise[..., :3], reduction='mean')+0.2*F.l1_loss(trans_llm, gt_trajectory[..., :3], reduction='mean'))
                + 10 * (F.l1_loss(rot, noise[..., 3:9], reduction='mean')+0.2*F.l1_loss(rot_llm, gt_trajectory[..., 3:9], reduction='mean'))
            )
            if torch.numel(gt_openess) > 0:
                openess = layer_pred[..., 9:]
                openess_llm = act_pred[..., 9:]
                loss += (F.binary_cross_entropy_with_logits(openess, gt_openess)+0.2*F.binary_cross_entropy_with_logits(openess_llm, gt_openess))
            total_loss = total_loss + loss
        return total_loss

#3d relative transformer的结构
