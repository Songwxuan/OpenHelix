...
#整个3dda的框架
class DiffuserActorACTS(nn.Module):
    ...
    def forward(
        ...
        if act_pred == None:
            ...
            act_pred_llm, start_idx= act_pred
            for layer_pred in pred:
                trans = layer_pred[..., :3]
                rot = layer_pred[..., 3:9]
                trans_llm = act_pred_llm[..., :3]
                rot_llm = act_pred_llm[..., 3:9]
                loss = (
                    30 * F.l1_loss(trans, noise[..., :3], reduction='mean')
                    + 10 * F.l1_loss(rot, noise[..., 3:9], reduction='mean')
                )
                aux_loss = (
                    30 * (F.l1_loss(trans_llm, gt_trajectory[start_idx, 0, :3], reduction='mean'))
                    + 10 * (F.l1_loss(rot_llm, gt_trajectory[start_idx, 0, 3:9], reduction='mean'))
                )

                loss = 0.2*aux_loss + loss
                if torch.numel(gt_openess) > 0:
                    openess = layer_pred[..., 9:]
