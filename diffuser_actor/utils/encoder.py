...
#Encoder都在这儿，instruction token是由此处的encode_instruction调用self.instruction_encoder生成的
...
class Encoder(nn.Module):
    ...
    def vision_language_attention(self, feats, instr_feats):
        assert not torch.isnan(feats).any(), "Input feats has NaN"
        assert not torch.isnan(instr_feats).any(), "Input instr_feats has NaN"
        ...
        assert not torch.isnan(feats).any(), "Output feats has NaN"
        return feats
