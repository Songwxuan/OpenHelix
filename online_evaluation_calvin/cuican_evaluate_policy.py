...
def rollout(env, model, task_oracle, subtask, lang_annotation):
    ...
    for step in pbar:
        ...
        with torch.cuda.amp.autocast():
            start_time_onestep = time.time()
            trajectory = model.step(obs, lang_embeddings)
            end_time_onestep = time.time()
            elapsed_time_onestep = end_time_onestep - start_time_onestep
            # print(f"The onestep time is: {elapsed_time_onestep:.6f} seconds.")
        for act_ind in range(min(trajectory.shape[1], EXECUTE_LEN)): # min(19, 20) -> range(19) 执行19次
